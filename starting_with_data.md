...
--Exploring the duplicated record.
...
Answer: 
The country with the highest total revenue level is the United States. The highest NON NULL city is San Fransisco. The important thing to note is that the highest "city" is actually NULL/Not available in the database and so bsed on the data that is available since we cannot know if all the NULL values represent the same city or not is San Fransisco.
